...
def is_bad_report_sentence(sentence, row=None):
    if len(sentence) > 260:
        return True
    if re.search(r"(니다|니까|됩니다|합니다|입니다|겁니다|이에요|예요|어요|아요|죠)[.!?。]?$", sentence):
        return True
...
def normalize_report_tone(text):
    ...
    text = re.sub(r"([가-힣]{2,})았죠\.?", r"\1았음.", text)
    text = re.sub(r"([가-힣]{2,})이죠\.?", r"\1.", text)
    text = re.sub(r"([가-힣]{2,})죠\.?", r"\1.", text)
